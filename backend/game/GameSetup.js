...
module.exports = (io, socket) => {
    ...
    const joinRoom = ({ playerName, room }) => {
        ...
        if(roomSize <= 5){
            ...
            socket.emit('error', {message: 'Max Players have joined, please wait till the next game starts.'})
        }
    }
    ...
    const selectFish = ({num, playerName, code}) => {
        players.find((player, i) => {
            if (player.playerName === playerName) {
                players[i] = { playerName: playerName, choice : num, toggle : num, eye : false }
